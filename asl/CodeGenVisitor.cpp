...
antlrcpp::Any CodeGenVisitor::visitArrayElement(AslParser::ArrayElementContext *ctx) {
  ...
  std::string addrIdent = codAtID.addr;
  instructionList & codeIdent = codAtID.code;
  ...
  if (Symbols.isLocalVarClass(addrIdent)) {
    code = code || instruction::LOADX(temp, addrIdent, addrExpr);
  }
  else {
    std::string temp2 = "%"+codeCounters.newTEMP();
    code = code || instruction::LOAD(temp2, addrIdent) || instruction::LOADX(temp, temp2, addrExpr);
  }
...
antlrcpp::Any CodeGenVisitor::visitLeft_expr(AslParser::Left_exprContext *ctx) {
  ...
  instructionList &         codeId    = codAtIdent.code;
  ...
  if (ctx->expr()) {
    ...
    offs = codAtsExpr.addr;
    instructionList & codeExpr = codAtsExpr.code;

    code = code || codeExpr;

    if (Symbols.isParameterClass(addrIdent)) {
      std::string temp = "%" + codeCounters.newTEMP();
